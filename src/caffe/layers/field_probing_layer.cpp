...
void FieldProbingLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  std::vector<int> slided_filters_shape;
  slided_filters_shape.push_back(num_sliding_);
  slided_filters_shape.push_back(num_sliding_);
  slided_filters_shape.push_back(num_sliding_);
  slided_filters_shape.push_back(num_curve_);
  slided_filters_shape.push_back(len_curve_);
  slided_filters_shape.push_back(4);
  slided_filters_.Reshape(slided_filters_shape);
}

template<typename Dtype>
void FieldProbingLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  std::vector<int> top_shape = slided_filters_.shape();
  for (int i = 0; i < field_num_; ++i) {
...
void FieldProbingLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  ...
  const Dtype* probing_curves = this->blobs_[0]->cpu_data();
  const Dtype* transformations = transform_ ? (bottom[field_num_]->cpu_data()) : (NULL);
  for (int sample_idx = 0; sample_idx < num_samples; ++sample_idx) {
    int p_offset = sample_idx * 4;
    Dtype px = probing_curves[p_offset + 0];
    Dtype py = probing_curves[p_offset + 1];
    Dtype pz = probing_curves[p_offset + 2];
      ...
      for (int j = 0; j < num_sliding_; ++j) {
        ...
        for (int k = 0; k < num_sliding_; ++k) {
          ...
          for (int batch_idx = 0; batch_idx < batch_size_; ++batch_idx) {
            if (transform_) {
              const Dtype* t = transformations + batch_idx * len_trans_params;
            ...
            int top_offset = batch_idx * slided_num_samples + count_sliding * num_samples + sample_idx;
...
void FieldProbingLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
  ...
  caffe_set(this->blobs_[0]->count(), Dtype(0), bottom[0]->mutable_gpu_diff());
          ...
          for (int batch_idx = 0; batch_idx < batch_size_; ++batch_idx) {
            ...
            for (int field_idx = 0; field_idx < field_num_; ++field_idx) {
              ...
              for (int channel_idx = 0; channel_idx < field_channels; ++channel_idx) {
                w_diff_x += top_diff[channel_idx] * gradients[3 * channel_idx + 0];
                w_diff_y += top_diff[channel_idx] * gradients[3 * channel_idx + 1];
                w_diff_z += top_diff[channel_idx] * gradients[3 * channel_idx + 2];
              }
              delete gradients;
            }
            ...
            if (transform_) {
              const Dtype* t = trans + batch_idx * len_trans_params;
              Dtype diff_x = t[0]*w_diff_x + t[4]*w_diff_y + t[8]*w_diff_z;
              Dtype diff_y = t[1]*w_diff_x + t[5]*w_diff_y + t[9]*w_diff_z;
              Dtype diff_z = t[2]*w_diff_x + t[6]*w_diff_y + t[10]*w_diff_z;
              ...
              t_diff[8] += x*w_diff_z;
              t_diff[9] += y*w_diff_z;
              t_diff[10] += z*w_diff_z;
              t_diff[11] += w_diff_z;

              filters_diff[p_offset + 0] += diff_x;
              filters_diff[p_offset + 1] += diff_y;
              filters_diff[p_offset + 2] += diff_z;
            }
          } /* batch_idx */
  ...
  if (transform_) {
    caffe_scal(bottom[field_num_]->count(), Dtype(1.0/(slided_num_samples*batch_size_*field_num_)), trans_diff);
  }
    ...
    while (true) {
      ...
      if (length <= max_span && length >= min_span) {
        for (int l = 0; l < len_curve_; ++l) {
          ...
          data[count * 4 + 0] = sample_x;
          data[count * 4 + 1] = sample_y;
          data[count * 4 + 2] = sample_z;
          data[count * 4 + 3] = 1.0;
        }
        count++;
      }
